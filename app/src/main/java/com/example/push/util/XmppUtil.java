...
//
    ...
    public static List<String> getConferenceServices(String server, XMPPConnection connection) throws Exception {
        ...
        for (Iterator<DiscoverItems.Item> it = items.getItems(); it.hasNext(); ) {
            ...
            Log.i("xmpp", "--id:" + item.getEntityID() + "--name:" + item.getName());
    ...
    public static void sendMessage(XMPPConnection mXMPPConnection, String content, String touser) throws XMPPException {
        ...
        Chat chat = chatmanager.createChat(touser + "@" + Constant.XMPP_HOST, null);
        if (chat != null) {
            chat.sendMessage(content);
            Log.i("xmpp", "给" + touser + "发送成功");
        }
    }
     ...
     * @throws XMPPException
     */
    public static void sendGroupMessage(XMPPConnection mXMPPConnection, String content, String groupJId) throws XMPPException {
        if (mXMPPConnection == null || !mXMPPConnection.isConnected()) {
            throw new XMPPException();
        }
        // 使用XMPPConnection创建一个MultiUserChat窗口
        MultiUserChat multiUserChat = new MultiUserChat(mXMPPConnection, groupJId);
        Collection<Occupant> students = multiUserChat.getParticipants();
        Log.i("xmpp", "学生个数：" + students.size());
        for (Occupant partic : students) {
            sendMessage(mXMPPConnection, content, partic.getNick());
        }
    }
